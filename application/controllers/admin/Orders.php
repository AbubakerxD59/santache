    ...
    public function update_orders()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($deliver_by) && !empty($deliver_by) && isset($orderid) && !empty($orderid)) {
                ...
                $settings = get_settings('system_settings', is_json: true);
            ...
            if (isset($orderid) && isset($field) && isset($val)) {
                ...
                if ($this->input->post('field', true) == 'status' && $update_status == 1) {
                    ...
                    } else if ($priority_status[$this->input->post('val', true)] == $priority_status[$current_orders_status]) {
                        $error = false;
                        $message = 'Order details updated successfully';
                    }
                }
                if ($error == true) {
                    $message = $msg . ' Order is already marked as ' . ($current_orders_status ?? 'cancelled') . '. You cannot set it to ' . $this->input->post('val', true) . '!';
                }
            }
