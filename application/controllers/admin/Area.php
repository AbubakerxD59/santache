...
class Area extends CI_Controller
{
    ...
    public function add_area()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (null !== $edit_area) {
                    if (is_exist(['name' => $this->input->post('area_name', true), 'city_id' => $this->input->post('city', true), 'zipcode_id' => $this->input->post('zipcode', true)], 'areas', array($this->input->post('edit_area', true)))) {
                        $response["error"]   = true;
    ...
    public function add_city()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->form_validation->set_rules('city_name', 'City Name', 'trim|required|xss_clean|regex_match[/^\S+$/]');
            $this->form_validation->set_rules('minimum_free_delivery_order_amount', ' Minimum Free Delivery Amount ', 'trim|required|numeric|xss_clean|greater_than_equal_to[0]');
            $this->form_validation->set_rules('delivery_charges', ' Delivery Charges ', 'trim|required|numeric|xss_clean|greater_than_equal_to[0]');
            $settings = get_settings('system_settings', true);
            ...
            if (!$this->form_validation->run()) {
                ...
                if (null !== $edit_city) {
                    if (is_exist(['name' => $this->input->post('city_name', true)], 'cities', array($this->input->post('edit_city', true)))) {
                        $response["error"] = true;
    ...
    public function add_zipcode()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->form_validation->set_rules('city', ' City ', 'trim|required|xss_clean|regex_match[/^\S+$/]');
            $this->form_validation->set_rules('minimum_free_delivery_order_amount', ' Minimum Free Delivery Amount ', 'trim|required|numeric|xss_clean|greater_than_equal_to[0]');
            $this->form_validation->set_rules('delivery_charges', ' Delivery Charges ', 'trim|required|numeric|xss_clean|greater_than_equal_to[0]');
            $settings = get_settings('system_settings', true);
            if (isset($settings['pincode_wise_deliverability']) && $settings['pincode_wise_deliverability'] == 1) {
                $this->form_validation->set_rules('zipcode', ' Zipcode ', 'trim|required|xss_clean|regex_match[/^\S+$/]');
            }
            if (!$this->form_validation->run()) {
                $this->response['error'] = true;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                ...
                if (isset($settings['pincode_wise_deliverability']) && $settings['pincode_wise_deliverability'] == 1) {
                    $edit_zipcode = $this->input->post('edit_zipcode', true);
                    if (null !== $edit_zipcode) {
                        if (is_exist(['city_id' => $this->input->post('city', true), 'zipcode' => $this->input->post('zipcode', true)], 'zipcodes', array($this->input->post('edit_zipcode', true)))) {
                            $response["error"]   = true;
                            $response["message"] = "Combination Already Exist ! Provide a unique Combination";
                            $response['csrfName'] = $this->security->get_csrf_token_name();
                            $response['csrfHash'] = $this->security->get_csrf_hash();
                    ...
                    $edit_zipcode = $this->input->post('edit_zipcode', true);
                    if (null !== $edit_zipcode) {
                        if (is_exist(['city_id' => $this->input->post('city', true), 'zipcode' => ''], 'zipcodes', array($this->input->post('edit_zipcode', true)))) {
                            $response["error"]   = true;
                            $response["message"] = "Combination Already Exist ! Provide a unique Combination";
                            $response['csrfName'] = $this->security->get_csrf_token_name();
                            $response['csrfHash'] = $this->security->get_csrf_hash();
