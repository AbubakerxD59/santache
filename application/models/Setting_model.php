...
class Setting_model extends CI_Model
{
    ...
    public function update_system_setting($post)
    {
        ...
        $system_data = [
            ...
            'custom_charges_pos' => (isset($post['custom_charges_pos'])) ? '1' : '0',
            'custom_charges_doorstep' => (isset($post['custom_charges_doorstep'])) ? '1' : '0',
            'custom_charges_pickup' => (isset($post['custom_charges_pickup'])) ? '1' : '0',
             'ai_settings_status' => (isset($post['ai_settings_status'])) ? '1' : '0',
                    'ai_provider' => isset($post['ai_provider']) ? $post['ai_provider'] : 'gemini',
   ...
   public function update_custom_charges($json)
{
    // Decode incoming custom charges
    $custom_charges = json_decode($json, true);
    if (!is_array($custom_charges)) {
        $custom_charges = [];
    }

    // ----------------------------------------
    // REMOVE DUPLICATES + NORMALIZE DATA
    // ----------------------------------------
    ...
    foreach ($custom_charges as $charge) {
        ...
        // Normalize name
        $name = strtolower(trim(str_replace(' ', '_', $charge['name'])));
        ...
        // Normalize apply flags (IMPORTANT)
        $filtered[] = [
            'name' => $name,
            'amount' => isset($charge['amount']) ? (float)$charge['amount'] : 0,

            // force 0 / 1 values
            'apply_pos' => !empty($charge['apply_pos']) ? 1 : 0,
            'apply_doorstep' => !empty($charge['apply_doorstep']) ? 1 : 0,
            'apply_pickup' => !empty($charge['apply_pickup']) ? 1 : 0,
            'apply_digital' => !empty($charge['apply_digital']) ? 1 : 0,
            'is_refundable' => !empty($charge['is_refundable']) ? 1 : 0,
        ];
    }
    ...
    $exists = $this->db
        ->get_where('settings', ['variable' => 'custom_charges'])
        ->row();

    if ($exists) {
        $this->db
            ->where('variable', 'custom_charges')
            ->update('settings', ['value' => $final_json]);
    } else {
        $this->db->insert('settings', [
            'variable' => 'custom_charges',
            'value' => $final_json
        ]);
    }
}

    public function get_custom_charges()
    {

        $row = $this->db->get_where('settings', ['variable' => 'custom_charges'])->row();

        if ($row) {
            return isset($row->value) ? json_decode($row->value, true) : '';
        }
    ...
    public function update_time_slot($post)
    {
        ...
        $edit_id = (isset($post['edit_time_slot']) && !empty($post['edit_time_slot'])) ? $post['edit_time_slot'] : (isset($post['id']) ? $post['id'] : '');
        
        if (!empty($edit_id)) {
            $this->db->set($time_slot_data)->where('id', $edit_id)->update('time_slots');
        } else {
    ...
    public function get_time_slot_details()
    {
        ...
        foreach ($search_res as $row) {
            ...
            $tempRow['from_time'] = date('h:i A', strtotime($row['from_time']));
            $tempRow['to_time'] = date('h:i A', strtotime($row['to_time']));
            $tempRow['last_order_time'] = date('h:i A', strtotime($row['last_order_time']));
