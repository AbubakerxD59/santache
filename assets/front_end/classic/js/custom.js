...
$(document).on('submit', '.form-submit-event', function (e) {
    ...
    // Remove name attribute from hidden form fields to prevent duplicate submissions
    var $form = $(this);
    var $hiddenInputs = $form.find('input[type="hidden"]');
    var $hiddenEmailForm = $form.find('#login-email-form');
    var $hiddenMobileForm = $form.find('#login-mobile-form');

    // If email form is hidden, remove name from its input
    if ($hiddenEmailForm.hasClass('d-none')) {
        $hiddenEmailForm.find('input[name="identity"]').removeAttr('name');
    }
    // If mobile form is hidden, remove name from its input
    if ($hiddenMobileForm.hasClass('d-none')) {
        $hiddenMobileForm.find('input[name="identity"]').removeAttr('name');
    }
...
if (auth_settings == "firebase") {
     ...
     */
    // Use jQuery event delegation for form submission to work with dynamically added elements
    $(document).on('submit', '#send-otp-form', onSignInSubmit);

    // Phone number input listeners with event delegation
    $(document).on('keyup', '#phone-number', updateSignInButtonUI);
    $(document).on('change', '#phone-number', updateSignInButtonUI);

    window.onload = function () {
        var offerPopupValue = document.getElementById("offer_popup_value");
        if (offerPopupValue && (offerPopupValue.value == "refresh" || offerPopupValue.value == "session_storage")) {
...
$(function () {
                ...
                $.getJSON(
                    ...
                    function (data) {
                        ...
                        if (data.brand && data.brand !== '' && data.brand !== null) {
                            ...
                            $('.quickview-product-brands').removeClass('d-none');
                        } else {
                            $('.quickview-product-brands').addClass('d-none');
                        }
                        ...
                        $.each(data.variant_attributes, function (i, e) {
                            ...
                            $.each(attribute_ids, function (j, id) {
                                ...
                                if (swatche_types[j] == '1') {
                                    ...
                                    variant_attributes +=
                                        '<style> .product-page-details .btn-group>.active { border: 1px solid black;}</style>' +
                                        '<button class="btn fullCircle' + activeClass + '" ' +
                                        color_code +
                                        '>' +
                                        ...
                                        '" value="' +
                                        id +
                                        '" class="modal-product-attributes" autocomplete="off"' + checkedAttr + '><br>' +
                                        '</button>'
                                } else if (swatche_types[j] == '2') {
                                    is_image = 1
                                    variant_attributes +=
                                        '<style> .product-page-details .btn-group>.active { color: #000000; border: 1px solid black;}</style>' +
                                        '<label class="btn text-center bg-transparent' + activeClass + '">' +
                                    ...
                                    variant_attributes +=
                                        style1 +
                                        '<label class="btn btn-default text-center' + activeClass + '">' +
                                        '<input type="radio" name="' +
                                        e.attr_name +
                                        '" value="' +
                                        id +
                                        '" class="modal-product-attributes" autocomplete="off"' + checkedAttr + '>' +
                                        attribute_values[j] +
                                        '<br>' +
                                        '</label>'
                                }
                        ...
                        setTimeout(function () {
                            $('.modal-product-attributes:checked:first').trigger('change')
                        }, 100)
                    }
                )
            }
            ...
            if ($(this).prop('checked')) {
                ...
                if (selected_attributes.length == attributes_length) {
                    ...
                    if (is_variant_available) {
                        ...
                        var selected_variant_values = $('.modal-product-variants[data-id="' + selected_variant_id + '"]').attr('data-variant-values') || '';
    ...
    $('#modal-add-to-cart-button').on('click', function (e) {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (typeof result.csrfHash !== 'undefined') {
                    csrfHash = result.csrfHash;
                }
                ...
                if (result.error == false) {
                    ...
                    if (is_loggedin == 0) {
                        ...
                        var variant_values = $('#modal-add-to-cart-button').attr('data-variant-values') || '';
                        var cart_item = {
                            product_variant_id: product_variant_id.trim(),
                            name: title,
                            short_description: text_description,
                            qty: parseInt(qty),
                            image: image,
                            price: price.trim(),
                        ...
                        if (cart !== null && cart !== undefined) {
                            for (var i = 0; i < cart.length; i++) {
                                if (cart[i].product_variant_id == product_variant_id.trim()) {
...
$(document).ready(function () {
    ...
    $(document).on(
        ...
        function (e) {
            ...
            if (max && !isNaN(max) && qty > max) {
                Toast.fire({
                    icon: 'error',
                    title: `Oops! Maximum allowed quantity is ${max}`
                })
                $(this).val(max)
                return
            }
            ...
            if (qty % step == 0) {
                if (is_loggedin == 1) {
                    var formData = {};
                    formData['product_variant_id'] = id;
                    formData['qty'] = qty;
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        success: function (result) {
                            // Update CSRF token with new value from server
                            if (typeof result.csrfHash !== 'undefined') {
                                csrfHash = result.csrfHash;
                            }
                            if (result.error == false) {
                        ...
                        //     Toast.fire({
                        //         icon: 'error',
                        //         title: 'Error updating cart: ' + error
                        //     })
                ...
                if (is_loggedin == 1) {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            ...
                            if (typeof result.csrfName !== 'undefined') {
                                csrfName = result.csrfName;
                            }
                            ...
                            if (result.error == false) {
                                ...
                                cart_count--
                                $('#cart-count').text(cart_count > 0 ? cart_count : '')
                                removeItem(product)
                                if (cart_count <= 0) {
                                    $('#cart-item-sidebar').html('<h1 class="h4 text-center">Your Cart Is Empty</h1>')
                                }
                    ...
                    if (cart) {
                        ...
                        var cart_count = new_cart.length
                        $('#cart-count').text(cart_count > 0 ? cart_count : '')
    ...
    function updateQuantity(quantityInput, price) {
        ...
        var linePrice = itemPrice * quantity
...
$('.product_reset_btn').on('click', function (e) {
    e.preventDefault();
    location.href = base_url + 'products'; // reset to default page
});
...
$(document).on('click', '.add_to_cart', function (e) {
    ...
    var variant_values = $(this).attr('data-variant-values') || ''
    var min = $(this).attr('data-min')
    var max = $(this).attr('data-max')
    var step = $(this).attr('data-step')
    var btn = $(this)
    ...
    if (is_loggedin == 0 && (!price || price === 'undefined' || price === '')) {
        Toast.fire({
            icon: 'error',
            title: 'Price not available. Please select a variant.'
        })
        return
    }
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.error == false) {
                    ...
                    if (is_loggedin == 0) {
                        var cleanPrice = parseFloat((price || '0').toString().replace(/[^\d.-]/g, ''));

                        if (isNaN(cleanPrice) || cleanPrice <= 0) {
                            Toast.fire({
                                icon: 'error',
                                title: 'Invalid price. Please try again.'
                            })
                            return false;
                        }
                        ...
                        if (cart !== null && cart !== undefined) {
                            for (var i = 0; i < cart.length; i++) {
                                if (cart[i].product_variant_id == product_variant_id.trim()) {
                                    cart[i].qty = parseInt(cart[i].qty) + parseInt(min);
                                    itemExists = true;
                                    break;
                                }
                            }
...
function display_cart(cart) {
    ...
    if (cart !== null && cart.length > 0) {
        ...
        cart.forEach(item => {
            var priceValue = item.special_price || item.price || 0;
            if (typeof priceValue === 'string') {
                priceValue = priceValue.replace(/[^\d.-]/g, '');
            }
            var itemPrice = parseFloat(priceValue);
            var itemMax = item.max || item.total_allowed_quantity || 999;
            var itemMin = item.min || item.minimum_order_quantity || 1;
            var itemStep = item.step || item.quantity_step_size || 1;
            ...
            if (is_loggedin == 1 && item.product_variants && item.product_variants.length > 0) {
                if (item.product_variants[0] && item.product_variants[0].variant_values && item.product_variants[0].variant_values !== '') {
                    variantHtml = '<span class="d-block text-muted small">' + item.product_variants[0].variant_values.replace(/,/g, ' | ') + '</span>';
                }
            }
            else if (is_loggedin == 0 && item.variant_values && item.variant_values !== '' && item.variant_values !== 'undefined') {
                variantHtml = '<span class="d-block text-muted small">' + item.variant_values.replace(/,/g, ' | ') + '</span>';
            }
            ...
            html +=
                ...
                currency +
                ' ' +
                (itemQty * itemPrice).toLocaleString(undefined, {
        ...
        if (is_loggedin == 0 && validCart.length !== cart.length && validCart.length > 0) {
            localStorage.setItem('cart', JSON.stringify(validCart));
            $('#cart-count').text(validCart.length);
        } else if (is_loggedin == 0 && validCart.length === 0) {
...
function cart_sync() {
    ...
    // Parse and clean the cart data before sending
    try {
        var cartData = JSON.parse(cart);
        // Clean price values - remove currency symbols and ensure numeric
        cartData = cartData.map(function (item) {
            var cleanedItem = Object.assign({}, item);
            // Clean price field
            if (typeof cleanedItem.price === 'string') {
                cleanedItem.price = parseFloat(cleanedItem.price.replace(/[^\d.-]/g, ''));
            }
            // Ensure numeric fields are numbers
            cleanedItem.qty = parseInt(cleanedItem.qty) || 1;
            cleanedItem.min = parseInt(cleanedItem.min) || 1;
            cleanedItem.max = parseInt(cleanedItem.max) || 100;
            cleanedItem.step = parseInt(cleanedItem.step) || 1;
            return cleanedItem;
        });
    ...
    return $.ajax({
        ...
        success: function (result) {
            ...
            if (result.error == false) {
                ...
                // })
                localStorage.removeItem('cart')
                // Update cart count to 0 after successful sync
                $('#cart-count').text('');
            } else {
                // Toast.fire({
                //     icon: 'error',
                //     title: result.message || 'Failed to sync cart'
                // })
            }
        },
        error: function (xhr, status, error) {
            ...
            Toast.fire({
                icon: 'error',
                title: 'Failed to sync cart. Please try again.'
            })
        }
    })
}
...
$(document).on('click', '.compare', function (e) {
    ...
    Toast.fire({
        icon: 'success',
        title: 'Added Successfully to Compare'
    })
    compare = compare !== null ? JSON.parse(compare) : null
...
$(document).on('click', '.confirmReturn', function (e) {
    ...
    $('.update-order').on('click', function (e) {
        ...
        if (confirm('Are you sure you want to 1234 ' + temp + ' this order ?')) {
            var t = $(this)
            var btn_text = t.text()
            formdata.append(csrfName, csrfHash)
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.error == false) {
                    ...
                    deleteButton.closest('.review-container').remove()
...
$(document).on('submit', '#ticket_form', function (e) {
    ...
    if (!type || !email || !subject || !description) {
        Toast.fire({
            icon: 'error',
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.error === false) {
                Toast.fire({
                    icon: 'success',
                    title: result.message || 'Ticket submitted successfully'
                });
        ...
        complete: function () {
            $('.ask_question').prop('disabled', false).text('Save');
        }
    });
});
    ...
    $(document).on('click', '.update_question', function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                var successCondition = (isError === false || isError === 0 || isError === '0' || isError === 'false' || !isError);
                if (successCondition) {
                    Toast.fire({
                        icon: 'success',
                        title: result.message || 'Saved successfully'
                    })
                    setTimeout(function () {
                        location.reload()
                    }, 600)
...
$(document).ready(function () {
    ...
    function completeSocialLogin(loginResult) {
        if (typeof loginResult.csrfHash !== 'undefined') {
            csrfHash = loginResult.csrfHash;
        }
        if (typeof loginResult.csrfName !== 'undefined') {
            csrfName = loginResult.csrfName;
        }
        if (loginResult.error == false) {
            ...
            Toast.fire({
                icon: 'error',
                title: loginResult.message || 'Login failed. Please try again.'
            });
        }
    }
